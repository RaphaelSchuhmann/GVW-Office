        ...
        <div class="flex-1 min-h-0 overflow-y-auto w-full">
            <div class="flex flex-col items-center gap-5 min-[1500px]:w-1/2 min-[1200px]:w-2/3 w-full mt-5">
                ...
                {:else}
                    ...
                    <Input title="Titel" placeholder="The Final Countdown" bind:value={draft.title} marginTop="5" />
                    <Input title="Komponist/Band" placeholder="Europe" bind:value={draft.artist} marginTop="5" />
                    <Dropdown title="Kategorie" options={getLibraryCategories(false)} selected={appSettings.scoreCategories[draft.type]}
                              onChange={(value) => draft.type = appSettings.scoreCategories[value]} marginTop="5" />
                    <TabBar contents={["Männerchor", "Gemischterchor"]} selected={editedSelectedChoirType}
                            onChange={(value) => {editedSelectedChoirType = value; !editTabBarInitialized ? editTabBarInitialized = true : draft.voices = [];}} marginTop="5" />
