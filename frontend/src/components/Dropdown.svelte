...
<div class={`flex flex-col items-start w-full ${marginMap[marginTop]} gap-1`} bind:this={dropdownRef}>
    ...
    <div class="relative inline-block w-full">
        <button
            class={`flex items-center w-full bg-gv-input-bg ${open && options.length > 0 ? "rounded-t-1" : "rounded-1"} text-dt-6 p-3 cursor-pointer text-gv-dark-text hover:bg-gv-hover-effect`}
            on:click={() => open = !open}>
        ...
        {#if open && options.length > 0}
            <div class="absolute w-full bg-gv-input-bg flex flex-col items-center rounded-b-1">
